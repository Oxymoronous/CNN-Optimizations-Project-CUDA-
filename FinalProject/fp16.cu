...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    //Revision 1 (simplified version)
    int m = blockIdx.x;
    int W_size = ceil(Width_out/(1.0*TILE_WIDTH));  //variable is needed because not passed in as parameter
    ...
    for (int c = 0; c < Channel; c++){
        for (int p  = 0; p < K; p++) {
            for (int q = 0; q < K; q++) {
                __half a = __float2half(in_4d(b, c, h+p, w+q));
                __half b = __float2half(mask_4d(m, c,p, q));
                __half c = __hmul(a, b);
                acc = __hadd(acc, c);
               //__half2 a = __floats2half2_rn(in_4d(b, c, h+p, w+q), in_4d(b, c, h+p,w+q+1));    
               //__half2 b = __floats2half2_rn(mask_4d(m, c, p, q), mask_4d(m, c, p, q+1)); 
               //__half2 c = __hmul2(a, b);
                //acc = __hadd2(acc, c);
               //mult = __high2half2(__float2half2_rn(in_4d(b, c, h + p, w + q)))* __high2half2(__float2half2_rn(mask_4d(m, c, p, q));
               //acc = hadd2(acc, mult);
            }
        }
    }
    if (h < Height_out && w < Width_out){
        ans = __half2float(acc); 
        out_4d(b, m, h, w) = ans;
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    dim3 gridDim(Map_out, y, Batch);  //number of samples in the batch
